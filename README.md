...
The Unix installer prefers a tagged release binary, verifies checksums when available, and falls back to `cargo build --release` from source for unsupported devices. It places the binary in `~/.local/bin` (`$HOME\.local\bin` on Windows). Set `OINO_PREFIX` to install somewhere else, `OINO_REF=vX.Y.Z` to pin a tag, and `OINO_EXTENSIONS=none|all|9router,vcc` to control built-in extension installation.
