...
- **UX visibility:** `/extensions` exposes management state, but install/update/remove commands are service-level APIs rather than polished end-user flows.
...
2. Add polished CLI/TUI install/update/remove flows backed by `PackageLifecycleService`.
